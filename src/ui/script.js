let cookiePath = null;
  ...
  function fmtRobux(n) { return `R$${fmt(Math.round((Number(n) || 0) * 100) / 100)}`; }
  function fmtUSD(n) {
    const v = Number(n ?? 0);
    return `$${v.toFixed(2)}`;
  }
  ...
  function showInsights(ins) {
    ...
    peakMonthLabelEl.textContent =
      `${periodToPretty(peak.period)} • ${fmt(peak.purchaseCount)} purchase${peak.purchaseCount === 1 ? "" : "s"}`;
  ...
  function drawChart() {
    ...
    for (let g = 0; g <= gridN; g++) {
      ...
      const label = chartMetric === "usd"
        ? `$${v.toFixed(vMax < 10 ? 2 : 0)}`
    ...
    if (hoverIndex >= 0 && hoverIndex < points.length) {
      ...
      ctx.font = "12px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Ubuntu, Cantarell, Noto Sans, sans-serif";
      const line1 = label;
      const line2 = vText;
    ...
    for (let i = 0; i < series.length; i += step) {
      const x = xFor(i);
      const text = chartGranularity === "year"
        ? String(series[i].period)
        : String(series[i].period).slice(2);
    ...
    if (series.length > 1) {
      ...
      const x = xFor(last);
      const text = chartGranularity === "year"
        ? String(series[last].period)
        : String(series[last].period).slice(2);
      ctx.fillText(text, x, padT + plotH + 8);
    }
